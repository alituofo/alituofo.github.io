...
void test02()
{
Person<string, int> p("猪八戒", 999); //类模板中的模板参数列表 可以指定默认参数
p.showPerson();
}
...
while (!q.empty()) {
//输出队头元素
cout << "队头元素-- 姓名： " << front().m_Name << " 年龄： " << front().m_Age;

cout << "队尾元素-- 姓名： " << back().m_Name << " 年龄： " << back().m_Age;
...
{
set<int> s;
pair<int>::iterator, bool>  ret = s.insert(10);
if (ret.second) {
cout << "第一次插入成功!" << endl;
}
else {
...
void test01()
{
pair<int> p(string("Tom"), 20);
cout "姓名： " " 年龄： " 

pair<int> p2 = make_pair("Jerry", 10);
cout "姓名： " " 年龄： " 
}
